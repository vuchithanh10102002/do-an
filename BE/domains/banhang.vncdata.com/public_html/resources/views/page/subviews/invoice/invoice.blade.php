...
<script>
    ...
    var cb_user = $('#user');
    var cb_state = $('#state');
    var btn_delete = $('<button><i class="fa fa-trash"></i> Xóa hóa đơn</button>');
    var branches = null;
    var customers = null;
    var users = null;

    invoiceApp.createData = function () {
        app.loadding('open');
        var downloaded = 0;
        ...
        function download_completed() {
            ...
            if (downloaded >= 2) {
                ...
                var states = [
                    ...
                    {code: 'ERROR', name: 'Lỗi lưu'},
                    {code: 'SENDING', name: 'Giao một phần'},
    ...
    invoiceApp.createUI = function () {
        ...
        function createToolbar(toolbar) {
            ...
            container_toolbar.append($("<div class='table-title'><span>Quản lý hóa đơn bán hàng</span></div>"));
        ...
        table_source = {
            localdata: null, datatype: "array", datafields: [
                {name: 'ngayhd', type: 'date',format: 'dd/MM/yyyy',},
                {name: 'id', type: 'string'},
        ...
        table.jqxGrid({
            ...
            columns: [
                ...
                {text: 'Mã KH', dataField: 'makh', width: '100'},
                {
                    text: 'Tên khách hàng', dataField: 'tenkh', width: '180',
                    aggregatesrenderer: function (aggregates, column, element) {
                ...
                {text: 'Ghi chú', cellsalign: 'center', align: 'center', dataField: 'ghichu'},
    ...
    invoiceApp.createEvent = function () {
        //Filter
        ...
        btn_delete.click(function () {
            ...
            swal({
                title: 'Bạn có chắc?',
                text: "Xóa hóa đơn, khôi phục sản phẩm và xóa các dữ liệu liên quan?",
                type: 'question',
                showCancelButton: true,
                confirmButtonText: 'Xóa ngay',
                cancelButtonText: 'Hủy bỏ',
            }).then(function () {
                delete_data(urls.url_apis.invoices, data_delete['id'], function (result) {
                    if (result['success']) {
                        app.notify("Xóa hóa đơn thành công!", 'success');
                        table.jqxGrid('deleterow',result.sent.index.uid);
                    } else {
                        app.notify('Xóa hóa đơn không thành công!', 'error');
                    }
                }, data_delete);
            }).catch(swal.noop);
    ...
    function view_start() {
        invoiceApp.createData();
        invoiceApp.createUI();
        invoiceApp.createEvent();
    }
</script>