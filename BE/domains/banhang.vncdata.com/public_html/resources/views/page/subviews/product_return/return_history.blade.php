...
<script>
    ...
    var cb_user = $('#user');
    var cb_state = $('#state');
    var btn_delete = $('<button><i class="fa fa-trash"></i> Xóa phiếu trả hàng</button>');
    var branches = null;
    var customers = null;
    var users = null;

    returnHistoryAPP.createData = function () {
        app.loadding('open');
        var downloaded = 0;
    ...
    returnHistoryAPP.createUI = function () {
        //
        ...
        function createToolbar(toolbar) {
            ...
            container_toolbar.append($("<div class='table-title'><span>Quản lý hóa đơn trả hàng</span></div>"));
    ...
    returnHistoryAPP.createEvent = function () {
        //Filter
        function request_filter() {
            ...
            download_data_where(urls.url_apis.select, {
                table: "invoices",
                select: "WHERE 1 AND type LIKE 'RETURN'" + string_where + ' ORDER BY id DESC',
    ...
    function view_start() {
        returnHistoryAPP.createData();
        returnHistoryAPP.createUI();
        returnHistoryAPP.createEvent();
    }
</script>