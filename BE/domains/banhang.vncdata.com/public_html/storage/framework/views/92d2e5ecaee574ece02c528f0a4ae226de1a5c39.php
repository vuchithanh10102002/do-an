    ...
    <div id="sidebar" style="width: 220px;">
        <div class="sidebar-item">
            ...
            <div>
                ...
                <div id="branch_receice" class="input-sidebar"></div>
...
<script>
    ...
    var cb_branch_receice = $('#branch_receice');
    var cb_user = $('#user');
    var cb_state = $('#state');
    var btn_delete = $('<button><i class="fa fa-trash"></i> Xóa phiếu chuyển</button>');
    var branches = null;
    var branch_receices = null;
    var users = null;

    moveHistoryAPP.createData = function () {
        app.loadding('open');
        var downloaded = 0;
        ...
        function download_completed() {
            ...
            if (downloaded >= 1) {
                ...
                branch_receices = JSON.parse(localStorage.getItem(tables.branches));

                branch_receices[branch_receices.length] = {
                    code: 'ALL',
                    name: 'Tất cả'
                };
    ...
    moveHistoryAPP.createUI = function () {
        //
        ...
        cb_branch_receice.jqxDropDownList({
            source: branch_receices,
            displayMember: "name",
            valueMember: "code",
            width: '100%',
            height: 30,
        ...
        function createToolbar(toolbar) {
            ...
            container_toolbar.append($("<div class='table-title'><span>Lịch sử chuyển hàng</span></div>"));
        ...
        table.jqxGrid({
            ...
            columns: [
                ...
                {text: 'Mã DL', dataField: 'makh', width: '100'},
                {
                    text: 'Tên đại lý nhận', dataField: 'tenkh', width: '180',
                    aggregatesrenderer: function (aggregates, column, element) {
                ...
                {
                    text: 'Trạng thái', dataField: 'trangthai', width: '100', align: 'right',
    ...
    moveHistoryAPP.createEvent = function () {
        //Filter
        function request_filter() {
            var branch_f = cb_branch.jqxDropDownList('val');
            var branch_receice_f = cb_branch_receice.jqxDropDownList('val');
            ...
            if (branch_receice_f !== 'ALL') {
                branch_receice_f = " AND customer LIKE '" + branch_receice_f + "'";
            } else {
                branch_receice_f = '';
            }
            if (user_f !== 'ALL') {
            ...
            var string_where = pcode_f + code_f + dateE_f + dateS_f + branch_receice_f + branch_f + user_f + state_f;
            table.jqxGrid('showloadelement');
            download_data_where(urls.url_apis.select, {
                table: "invoices",
                select: "WHERE 1 AND type LIKE 'MOVE'" + string_where + ' ORDER BY id DESC',
        ...
        $('#branch_receice').on('change', function (event) {
            setTimeout(function () {
                request_filter();
            }, 500);
        });
        ...
        btn_delete.click(function () {
            ...
            swal({
                title: 'Bạn có chắc?',
                text: "Xóa phiếu chuyển hàng, khôi phục sản phẩm và xóa các dữ liệu liên quan?",
                type: 'question',
                showCancelButton: true,
                confirmButtonText: 'Xóa ngay',
                cancelButtonText: 'Hủy bỏ',
            }).then(function () {
                delete_data(urls.url_apis.product_move, data_delete['id'], function (result) {
                    if (result['success']) {
                        app.notify("Xóa phiếu chuyển hàng thành công!", 'success');
                        table.jqxGrid('deleterow', result.sent.index.uid);
                    } else {
                        app.notify('Xóa phiếu chuyển hàng không thành công!', 'error');
                    }
                }, data_delete);
            }).catch(swal.noop);
    ...
    function view_start() {
        moveHistoryAPP.createData();
        moveHistoryAPP.createUI();
        moveHistoryAPP.createEvent();
    }
</script>