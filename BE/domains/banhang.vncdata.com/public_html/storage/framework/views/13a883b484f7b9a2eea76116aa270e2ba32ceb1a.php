    ...
    <div id="sidebar" style="width: 220px;">
        <div class="sidebar-item">
            ...
            <div>
                ...
                <div id="supplier" class="input-sidebar"></div>
...
<script>
    ...
    var cb_supplier = $('#supplier');
    var cb_user = $('#user');
    var cb_state = $('#state');
    var btn_delete = $('<button><i class="fa fa-trash"></i> Xóa phiếu nhập</button>');
    var branches = null;
    var suppliers = null;
    var users = null;

    importHistoryAPP.createData = function () {
        app.loadding('open');
        var downloaded = 0;
    ...
    importHistoryAPP.createUI = function () {
        //
        ...
        function createToolbar(toolbar) {
            ...
            container_toolbar.append($("<div class='table-title'><span>Phiếu nhập hàng</span></div>"));
                ...
                download_data_where(urls.url_apis.select, {
                    ...
                    if (result['success']) {
                        ...
                        for (var i = 0; i < data_result.length; i++) {
                            data_result[i]['price'] = data_result[i]['price_import'];
                            data_result[i]['price_sale'] = parseInt(data_result[i]['price_import']);// / data_result[i]['number']);
    ...
    importHistoryAPP.createEvent = function () {
        //Filter
        function request_filter() {
            var branch_f = cb_branch.jqxDropDownList('val');
            var supplier_f = cb_supplier.jqxDropDownList('val');
            ...
            if (supplier_f !== 'ALL') {
                supplier_f = " AND customer LIKE '" + supplier_f + "'";
            } else {
                supplier_f = '';
            }
            if (user_f !== 'ALL') {
            ...
            var string_where = pcode_f + code_f + dateE_f + dateS_f + supplier_f + branch_f + user_f + state_f;
            table.jqxGrid('showloadelement');
            download_data_where(urls.url_apis.select, {
                table: "invoices",
                select: "WHERE 1 AND type LIKE 'IMPORT'" + string_where + ' ORDER BY id DESC',
        ...
        btn_delete.click(function () {
            ...
            swal({
                title: 'Bạn có chắc?',
                text: "Xóa phiếu nhập, khôi phục sản phẩm và xóa các dữ liệu liên quan?",
                type: 'question',
                showCancelButton: true,
                confirmButtonText: 'Xóa ngay',
                cancelButtonText: 'Hủy bỏ',
            }).then(function () {
                delete_data(urls.url_apis.product_import, data_delete['id'], function (result) {
                    if (result['success']) {
                        app.notify("Xóa phiếu nhập hàng thành công!", 'success');
                        table.jqxGrid('deleterow', result.sent.index.uid);
                    } else {
                        app.notify('Xóa phiếu nhập hàng không thành công!', 'error');
                    }
                }, data_delete);
            }).catch(swal.noop);
    ...
    function view_start() {
        importHistoryAPP.createData();
        importHistoryAPP.createUI();
        importHistoryAPP.createEvent();
    }
</script>